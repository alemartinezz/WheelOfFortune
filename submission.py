...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self,category,obscuredPhrase, guessed):
        prompt = input("Guess a letter, phrase, or type 'exit' or 'pass':")
        return prompt
            ...
            if i not in guessed:
                if i in VOWELS:
                    if self.prizeMoney < 250:
                        ...
                        self.can_be_guessed.append(i)
                else:
                    self.can_be_guessed.append(i)
        if self.can_be_guessed ==[]:
            return "pass"
        else:
            return self.can_be_guessed
        
 
    def getMove(self,category, obscuredPhrase, guessed):
        if self.getPossibleLetters(guessed) == "pass":
            return "pass"
        self.coin_value = self.smartCoinFlip()
        
        if self.coin_value == True: 
            for char in self.SORTED_FREQUENCIES[::-1]:
                if char in self.getPossibleLetters(guessed):
                    return char
        else:
            self.rand_letter = random.choice(self.getPossibleLetters(guessed))
            return self.rand_letter
import sys
